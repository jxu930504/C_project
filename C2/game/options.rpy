## 此文件包含可以更改以自訂您的遊戲的選項。
##
## 以兩個'#' 標記開頭的行是註釋，您不應取消註釋它們。以單一'#' 標記開頭的行是註
## 解掉的程式碼，您可能需要在適當的時候取消註解它們。
...
## 這三個變數控制預設向播放器顯示哪些混音器。將其中之一設為 False 將隱藏相應的混
## 音器。
...
define config.save_directory = "test-1774927480"
...
init python:
    ...
    ## 以下函式採用檔案模式。 檔案模式不區分大小寫，並與相對於基底目錄的路徑進行
    ## 匹配，無論是否有前導 /。 如果多個模式匹配，則使用第一個模式。
    ##
    ## 在一個模式中：
    ##
    ## / 是目錄分隔符號。
    ...
    ## ** 匹配所有字元，包括目錄分隔符。
    ##
    ## 就比如， "*.txt" 匹配 base 目錄中的txt文件， "game/**.ogg" 匹配遊戲目錄或
    ## 其任何子目錄中的 ogg 文件，和 "**.psd" 匹配專案中任意位置的 psd 檔案。
    ...
    ## 與文件模式匹配的文件在 Mac 應用程式建置中會重複，因此它們會同時出現在應用
    ## 程式和 zip 檔案中。
...
init python:
    # 定義一個函數來呼叫放大圖片的 Screen
    def show_enlarged_image(img_path):
        renpy.show_screen("enlarge_image", img_path)
        renpy.restart_interaction()

    # 將這個函數註冊為超連結處理器，命名為 "showimg"
    config.hyperlink_handlers["showimg"] = show_enlarged_image
## 執行應用程式內購買需要 Google Play 授權金鑰。它可以在 Google Play 開發者控制
## 台的 "Monetize" > "Monetization Setup" > "Licensing" 下找到。
